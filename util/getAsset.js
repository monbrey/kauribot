...
module.exports = async pokemon => {
    ...
    try {
        await request.head(
            `https://img.pokemondb.net/sprites/sun-moon/normal/${pokemon.uniqueName.toLowerCase()}.png`
        )
        assets.image = `https://img.pokemondb.net/sprites/sun-moon/normal/${pokemon.uniqueName.toLowerCase()}.png`
    } catch (e) {
        try {
            await request.head(
                `https://img.pokemondb.net/sprites/x-y/normal/${pokemon.uniqueName.toLowerCase()}.png`
            )
    ...
    try {
        await request.head(
            `https://img.pokemondb.net/sprites/sun-moon/icon/${pokemon.uniqueName.toLowerCase()}.png`
        )
