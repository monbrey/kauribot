...
pokemonSchema.methods.learnset = async function() {
    ...
    const moveCount = Object.values(this.moves).slice(1).reduce((acc, obj) => acc + (obj ? obj.length : 0), 0)
    const embed = new RichEmbed()
        .setTitle(`${this.displayName} can learn ${moveCount} move(s)`)

    const learnset = []
    const moves = new Collection(Object.entries(this.moves).slice(1).filter(m => m.length > 0))
