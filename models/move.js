...
moveSchema.methods.info = async function() {
    let embed = new RichEmbed()
        ...
        .setDescription(
            `
| Type: ${this.moveType} | Power: ${this.power ? this.power : '-'} | Accuracy: ${
    this.accuracy ? this.accuracy : '-'
} | PP: ${this.pp} | Category: ${this.category} |

${this.desc} ${this.contact ? 'Makes contact. ' : ''}${this.sheerForce ? 'Boosted by Sheer Force. ' : ''}${
    this.substitute ? 'Bypasses Substitute. ' : ''
}${this.snatch ? 'Can be Snatched. ' : ''}${this.magicCoat ? 'Can be reflected by Magic Coat. ' : ''}`
        )
        .setFooter(this.note || '')
    ...
    if (this.tm)
        embed.addField(
            'TM',
            `Taught by TM${this.tm.number.toString().padStart(2, 0)} ($${this.tm.martPrice.pokemart.toLocaleString()})`
        )
