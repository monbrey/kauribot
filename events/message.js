    ...
    async run(message) {
        ...
        if (command.config.channels.get(message.channel.id) === undefined) {
            if (command.config.guilds.get(message.guild.id) === undefined)
                return message.channel.send(`${message.client.prefix}${command.name} has not been configured for use in this server`)
            if (!command.config.guilds.get(message.guild.id))
                return message.channel.send(`${message.client.prefix}${command.name} has been disabled on this server`)
        } else if (!command.config.channels.get(message.channel.id)) {
            return message.channel.send(`${message.client.prefix}${command.name} has been disabled in this channel`)
        }
