    ...
    async run(reaction, user) {
        ...
        let getImage = async (message) => {
            if(message.attachments.size > 0) {
                if(/(http(s?):)([/|.|\w|\s|-])*\.(?:jpg|jpeg|gif|png)/gi.test(message.attachments.array()[0].url))
                    return message.attachments.array()[0].url
            }
            if(message.embeds.length > 0) {
                if(message.embeds[0].type === "image" && /(http(s?):)([/|.|\w|\s|-])*\.(?:jpg|gif|png)/gi.test(message.embeds[0].url))
