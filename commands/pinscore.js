    ...
    async run(message, args = [], flags = []) {
        ...
        let channels =
            message.mentions.channels.size > 0
                ...
                : new Collection().set(message.channel.id, message.channel)
        let messageSets = await Promise.all(
        ...
        messageSets.forEach(messages => {
            messages.forEach(m => {
                score[m.author.username] =
                    m.author.username in score ? score[m.author.username] + 1 : 1
            })
        })

        let newScore = Object.entries(score)
            .sort()
            .reduce(
                (_sortedObj, [k, v]) => ({
                    ..._sortedObj,
