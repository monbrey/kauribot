...
module.exports = class StartCommand extends BaseCommand {
    ...
    async getUsername(message, sentMessage, embed) {
        try {
            ...
            if (await Trainer.usernameExists(username)) {
                let warn = await message.channel.send(new RichEmbed().warning("Invalid Trainer name", "A trainer with that name already exists"))
                warn.delete(5000)
                return await this.getUsername(message, sentMessage, embed)
            } else if (!validator.isAscii(username)) {
                let warn = await message.channel.send(new RichEmbed().warning("Invalid Trainer name", "The trainer name provided contains invalid (non-ASCII) characters"))
                warn.delete(5000)
                return await this.getUsername(message, sentMessage, embed)
                ...
                let warn = await message.channel.send(new RichEmbed().warning("Invalid Trainer name", "Trainer names must be between 1 and 64 characters"))
                warn.delete(5000)
                return await this.getUsername(message, sentMessage, embed)
            } else return username
        } catch (e) {
            message.client.activeCommands.sweep(x => x.user === message.author.id && x.command === "start")
            message.client.logger.error(`Error prompting for username: ${e.stack}`, {
                key: "start"
            })
        }
    }
    ...
    async confirmUsername(message, sentMessage, embed) {
        try {
            ...
            message.client.activeCommands.sweep(x => x.user === message.author.id && x.command === "start")
            message.client.logger.error(`Error confirming username: ${e.stack}`, {
                key: "start"
            })
        }
    }
    ...
    async getStarter(message, sentMessage, embed) {
        try {
            ...
            message.client.activeCommands.sweep(x => x.user === message.author.id && x.command === "start")
            message.client.logger.error(`Error selecting starter: ${e.stack}`, {
                key: "start"
            })
        }
    }
    ...
    async confirmStarter(message, sentMessage, embed) {
        try {
            ...
            message.client.activeCommands.sweep(x => x.user === message.author.id && x.command === "start")
            message.client.logger.error(`Error confirming starter: ${e.stack}`, {
                key: "start"
            })
        }
    }
