...
#channel : Send logs to #channel`,
    ...
    async run(message, args = [], flags = []) {
        ...
        if (!target) {
            if (message.guild.logChannel) return message.channel.sendPopup("success", `Logs are being output to ${message.guild.logChannel}.`)
            else return message.channel.sendPopup("warn", "Logging is not configured for this server.")
        } else if (target.constructor.name !== "TextChannel")
            return message.channel.sendPopup("warn", "To set a destination for log output, you must mention a TextChannel")
        else {
            ...
            try {
                ...
                return message.channel.sendPopup("success", `Logs will be sent to ${target}. It is recommended that you prevent other users from sending messages to this channel.`, 0)
            } catch (e) {
                message.client.logger.parseError(e, "config")
                return message.channel.sendPopup("error", `Error updating logs configuration: ${e.message}`)
            }
        }
    }
}
