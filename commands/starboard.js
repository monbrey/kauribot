...
module.exports = class StarboardCommand extends BaseCommand {
    ...
    async getStarboard(message) {
        ...
        if (starChannel)
            return message.channel.sendPopup(
                'info',
                `The Starboard for this server is ${starChannel}.`
            )
        else
            return message.channel.sendPopup(
                'warn',
                'The Starboard is not configured for this server, or is missing.'
            )
    }
    ...
    async setStarboard(message, target) {
        ...
        try {
            ...
            return message.channel.sendPopup(
                'success',
                `Starboard channel has been set to ${target}. It is recommended that you prevent other users from sending messages to this channel.`
            )
        } catch (e) {
            message.client.logger.parseError(e, 'starboard')
            return message.channel.sendPopup(
                'error',
                `Error updating starboard configuration: ${e.message}`
            )
        }
    }

    async emoji(message, arg) {
        if (!message.guild.starboard)
            return message.channel.sendPopup('warn', 'This server has no Starboard')
        if (!arg) {
            if (message.guild.starboard.emoji)
                return message.channel.sendPopup(
                    'info',
                    `The Starboard emoji for this server is ${message.guild.starboard.emoji}.`
                )
            else
                return message.channel.sendPopup(
                    'warn',
                    'This setting requires an emoji to be provided'
                )
        }
    ...
    async reacts(message, arg) {
        ...
        if (!arg) {
            if (message.guild.starboard.minReacts)
                return message.channel.sendPopup(
                    'info',
                    `The Starboard minimum reactions for this server is ${
                        message.guild.starboard.minReacts
                    }.`
                )
            else
                return message.channel.sendPopup(
                    'warn',
                    'The Starboard is not configured for this server'
                )
        }
        if (!/^[1-9][0-9]*$/.test(arg))
            return message.channel.sendPopup(
                'warn',
                'This setting requires a positive integer to be provided'
            )
        ...
        try {
            ...
            return message.channel.sendPopup(
                'success',
                `Starboard minimum reactions has been set to ${arg}`
            )
        } catch (e) {
            message.client.logger.parseError(e, 'starboard')
            return message.channel.sendPopup(
    ...
    async run(message, args = [], flags = []) {
        ...
        if (flags.length === 0) {
            if (!arg) return this.getStarboard(message)
            else if (arg.constructor.name !== 'TextChannel')
                return message.channel.sendPopup(
                    'warn',
                    'To set a Starboard for this server, you must mention a TextChannel'
                )
