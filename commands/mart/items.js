...
const browseItems = async (message, sentMessage = null) => {
    let embed = new RichEmbed()
        ...
        .setDescription(
            `Click the number to browse the corresponding item category
...
const browseCategory = async (message, category = 0, sentMessage = null, subCategory = 0, _page = 1) => {
    ...
    const nextText = items.next
        ? `${subCategoryString} Page ${_page + 1} ->`
        : subCategory !== catMatches[category].length - 1
            ? `${catMatches[category][subCategory + 1]} ->`
            : ''
    const prevText = items.prev
        ? `<- ${subCategoryString} Page ${_page - 1}`
        : subCategory !== 0
            ? `<- ${catMatches[category][subCategory - 1]}`
            : ''
    ...
    try {
        ...
        if (next === true)
            return items.next
                ? browseCategory(message, category, sentMessage, subCategory, _page + 1)
