...
const TrainerPokemon = require('../models/trainerPokemon')
...
module.exports = class RosterCommand extends BaseCommand {
    ...
    async roster(member, trainer) {
        const list = TrainerPokemon.find({ trainer: trainer.id })
        let plist1 = list.map((p, i) => `${i + 1}. ${p.basePokemon.displayName}`)
        let split = Math.ceil(plist1.length / 2)
        ...
        if (plist2.length > 0) embed.addField('\u200B', plist2.join('\n'), true)
    ...
    async run(message, args = [], flags = []) {
        ...
        try {
            ...
            let red = message.client.emojis.find(
                e => e.name === 'red' && message.client.emojiServers.includes(e.guild)
            )
            let backpack = message.client.emojis.find(
                e => e.name === 'backpack' && message.client.emojiServers.includes(e.guild)
            )
