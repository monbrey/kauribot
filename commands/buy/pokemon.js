...
const getCart = pokemon => {
    return `${
        pokemon.length > 0
            ? pokemon
                .map(p => {
                    let price = p.martPrice.pokemart
                        ? `$${p.martPrice.pokemart.toLocaleString()}`
                        : `${p.martPrice.berryStore.toLocaleString()} CC`
...
const updateCart = async (message, pokemon, cart = null) => {
    let embed = new RichEmbed()
        ...
        .setDescription(
            stripIndent`Reply with the names of all Pokemon you wish to purchase, as they are shown in the Pokemart.
        ...
        .addField(
            'Subtotal',
            `$${getSubtotals(pokemon)[0].toLocaleString()} | ${getSubtotals(pokemon)[1].toLocaleString()} CC`
        )
...
const buyPokemon = async (message, args = [], cart = null) => {
    ...
    let [pValid, pInvalid] = pResult.reduce(
        ([pass, fail], item) => {
            return item.martPrice ? [[...pass, item], fail] : [pass, [...fail, item]]
        },
        [[], []]
    )
