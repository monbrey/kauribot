...
let buyAbility = async (message) => {
    ...
    if(locked.length === 0) {
        return message.channel.sendPopup("warn", `Your ${name} does not have any more Hidden Abilities that require unlocking`)
    }
    ...
    return (
        (response.first().emoji.name === "❌") ? async () => {
            ...
            if (4000 > message.trainer.cash) {
                cart.clearReactions()
                return message.channel.sendPopup("error", "You have insufficient cash to complete this purchase. Please remove some items and try again.")
            }
