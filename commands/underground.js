...
module.exports = class UndergroundCommand extends BaseCommand {
    constructor() {
        super({
            ...
            category: 'Game',
            description:
                'URPG\'s item lottery - explore the Underground for a chance at rare and valuables items!',
    ...
    async chooseItem(message, rolls, item) {
        ...
        const embed = new RichEmbed().setTitle('Underground result').setDescription(stripIndent`
            **Rolls:** ${rolls.join(' | ')}
        ...
        const filter = (reaction, user) =>
            [numEmoji[1], numEmoji[2]].includes(reaction.emoji.name) &&
    ...
    async resolve(message, rolls, ug) {
        try {
            ...
            if (result === true) return Digs.addDig(message.trainer.id, result.id)
                ...
                const embed = new RichEmbed().setTitle('Underground result pending')
...
\`!ug claim ${result.url}\``)
    ...
    async run(message, args = [], flags = []) {
        ...
        message.trainer = await Trainer.findById(message.author.id).catch(e =>
            logger.parseError(e, this.name)
        )
        if (!message.trainer)
        ...
        if (args[0] === 'claim' && args[1]) {
            ...
            try {
                if (!channel || !ugMsg)
                    return message.channel.sendPopup(
                        'warn',
                        'Unable to parse Underground Message URL'
                    )
                ...
                if (!pendingUg)
                    return message.channel.sendPopup(
                        'warn',
                        'No pending Underground item pickup matches that URL'
                    )
                if (pendingUg.trainer !== message.author.id)
                    return message.channel.sendPopup(
                        'warn',
                        'That pending Underground item pickup is not yours to claim!'
                    )
