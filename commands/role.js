...
module.exports = class RoleCommand extends BaseCommand {
    ...
    async init(client) {
        ...
        client.guilds.tap(async guild => {
            ...
            missing.forEach(m => {
                 ...
                 */
                let assigners = guild.roles
                    .filter(r => {
                        return (
                            r.hasPermission('MANAGE_ROLES', false, true) &&
                            m.comparePositionTo(r) < 0
                        )
                    })
                    .map(r => r.id)
    ...
    async addAssigners(message, dbRole) {
        try {
            ...
            let embed = new RichEmbed()
                ...
                .setDescription(
                    'Please list any additional roles, by name, who should be able to assign this role with `!role`. Multiple roles can be separated by spaces.\n\nReact with ✅ when all roles have been listed - you will then be able to select roles to remove.\nReact with ❌ to cancel and make no changes.'
                )
                .setAuthor('Professor Kauri', 'https://cdn.discordapp.com/embed/avatars/0.png')
                .addField('Roles:', `\`\`\`${assignerNames.join(' ')}\`\`\``)
            ...
            return message.client.logger.error(
                `Error configuring "${dbRole.roleName}": ${e.stack}`,
                {
                    key: 'role'
                }
            )
    ...
    async run(message, args = [], flags = []) {
        let roleName =
            flags.indexOf('c') == 0 ? args.join(' ') : args.slice(0, args.length - 1).join(' ')
